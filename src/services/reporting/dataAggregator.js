...
async function getTopRootCauses(options = {}, limit = 10) {
  ...
  if (minConfidence !== undefined && minConfidence !== null) {
    ...
    whereConditions.push(`lrc.confidence >= $${++paramCount}`);
  }
  ...
  const query = `
    WITH latest_root_causes AS (
      ...
      WHERE jrc.root_cause_id IS NOT NULL
...
/**
 * Get failed jobs without identified root causes
 * @param {Object} options - Filter options
 * @returns {Promise<Array>} Jobs without root causes
 */
async function getJobsWithoutRootCause(options = {}) {
  const { startDate, endDate, workflows, repository } = options;

  let whereConditions = ['j.conclusion = \'failure\''];
  let params = [];
  let paramCount = 0;
  ...
  const query = `
    ...
    SELECT 
      ...
      wr.workflow_file_name,
      wr.head_branch,
      wr.html_url as run_html_url,
      wr.created_at as run_created_at,
      js.step_name as failed_step_name,
      js.step_number as failed_step_number
    FROM jobs j
    JOIN workflow_runs wr ON j.run_id = wr.run_id
    LEFT JOIN latest_root_causes lrc ON j.job_id = lrc.job_id
    LEFT JOIN LATERAL (
      ...
      FROM job_steps
      WHERE job_id = j.job_id AND conclusion = 'failure'
      ORDER BY step_number
      LIMIT 1
    ) js ON true
    ${whereClause}
      AND lrc.job_id IS NULL
    ORDER BY wr.created_at DESC, j.job_id
  `;
